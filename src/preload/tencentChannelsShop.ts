...
function getAddressList() {
  return new Promise((resolve, reject) => {
    fetch("https://m.piaoxingqiu.com/cyy_gatewayapi/user/buyer/v3/user/addresses?src=WEB&ver=4.2.1-20240322110808", {
      "headers": {
        "accept": "*/*",
...
async function getRemainingTicketCount(showId, sessionId) {
  let resData = {
    sessionId: sessionId,
    planCountMaps: {}
  }
...
function getSPUSaleShowSeatPlans(spuId) {
  return new Promise((r, j) => {
    fetch(
      `https://m.piaoxingqiu.com/cyy_gatewayapi/show/pub/v5/show/${spuId}/sessions?src=WEB&ver=4.2.1-20240322110808&source=FROM_QUICK_ORDER&isQueryShowBasicInfo=true`,
      {
        headers: {
          accept: "*/*",
          "accept-language": "zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6",
      ...
      .then(async (res) => {
        if (res.statusCode === 200) {
          ...
          const allCount = await Promise.all(res.data.map(session => getRemainingTicketCount(spuId, session.bizShowSessionId)))
          allCount.map(i => {
...
async function getDeliverMethod(spuId, sessionId, seatPlanId, price, qty) {
  const headers = {
    'User-Agent': 'Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.75 Mobile Safari/537.36',
    'Content-Type': 'application/json',
    'access-token': getToken()
  ...
  const url = "https://m.piaoxingqiu.com/cyy_gatewayapi/trade/buyer/order/v3/pre_order";
  try {
    const response = await fetch(url, {
      method: 'POST',
      headers: headers,
    ...
    if (responseData.statusCode === 200) {
      return responseData.data.supportDeliveries[0].name;
    } else {
      throw new Error("获取门票类型异常:" + JSON.stringify(responseData));
    }
  } catch (error) {
    console.error("获取门票类型异常:" + error);
    return '';
  }
}
...
function getAudienceList() {
  return new Promise((resolve, reject) => {
    fetch("https://m.piaoxingqiu.com/cyy_gatewayapi/user/buyer/v3/user_audiences?length=500&offset=0&src=WEB&ver=4.2.1-20240322110808", {
      "headers": {
        "accept": "*/*",
        "accept-language": "zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6",
        "access-token": getToken(),
...
function getUserInfo() {
  return new Promise((resolve, reject) => {
    fetch("https://m.piaoxingqiu.com/cyy_gatewayapi/user/buyer/v3/profile?src=WEB&ver=4.2.1-20240322110808", {
      "headers": {
        "accept": "*/*",
        "accept-language": "zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6",
        "access-token": getToken(),
...
function getSPUInfo(spuId) {
  return new Promise((resolve, reject) => {
    fetch(`https://m.piaoxingqiu.com/cyy_gatewayapi/show/pub/v5/show/${spuId}/static?src=WEB&ver=4.2.1-20240322110808&cityId=4401&source=FROM_QUICK_ORDER`, {
      "headers": {
        "accept": "*/*",
        "accept-language": "zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6",
        "access-token": getToken(),
...
<br />`
  ...
  async function checkDeliverMethod(sessionId, seatPlanId, price) {
    if (!deliver) {
      deliver = await getDeliverMethod(spuId, sessionId, seatPlanId, price, 1)
    }
  }

  async function createOrderParams({ plan, address, audience, spu }) {
    if (plan) {
      await checkDeliverMethod(plan?.session?.bizShowSessionId, plan?.plan?.seatPlanId, plan?.plan?.originalPrice)
    }
    return {
  ...
  return {
    actionCreateOrder: async () => {
      deliver = ""
      const params = await createOrderParams({
        plan: allPlan[activePlanIndex],
        address: allAddress[activeAddressIndex],
        audience: activeAudienceIndexList.map(audience => allAudience[audience]), spu: {
          spuId: spuId,
          token: getToken()
        }
      })
    ...
    actionLoopCreateOrder: async () => {
      deliver = ""
      async function startTimer(timerGap, isReadyPurchase) {
...
function actionCreateOrder(params, resultCallback) {
  ...
  let data = {
    "src": "WEB", // 固定值
    "ver": "4.2.1-20240322110808", // 固定值
    "addressParam": params.deliver === 'EXPRESS' ? { "addressId": addressId || "" } : {}, // 半定值，提前建好地址，并获取好 addressId
    ...
    "items": [
      {
        ...
        "deliverMethod": params.deliver
      }
    ],
    "priorityId": "",
    "many2OneAudience": params.deliver === "E_TICKET" ? {
      "audienceId": audienceList[0],
      "sessionIds": [
        sessionId
      ]
    } : {}
